...
if ($conn) {
    ...
    // If no current academic year found, get the latest one
    if (!$current_ay) {
        $current_ay_query = "SELECT Code, AcademicYear FROM academicyears 
                            ORDER BY StartDate DESC LIMIT 1";
        $current_ay_result = $conn->query($current_ay_query);
    ...
    if ($current_ay) {
        // 1. E-Clearance statistics for current academic year
        $stats_query = "
            SELECT 
        ...
        $stmt->bind_param("s", $current_ay['Code']);
        $stmt->execute();
        ...
        // 2. Students per department with clearance status for current academic year
        $dept_query = "
            SELECT 
                d.DepartmentName,
                COUNT(DISTINCT s.studentNo) as total_students,
...
<body>
    ...
    <div class="main-content">
        <div class="container">
            ...
            <div class="panel" style="margin-top: 20px; background-color: #f8f9fa; padding: 15px; border-radius: 5px;">
