...
if ($registration_no_result && $row = $registration_no_result->fetch_assoc()) {
    // Only update if max_registration is not null and is numeric
    if ($row['max_registration'] !== null && is_numeric($row['max_registration'])) {
        $next_registration_no = intval($row['max_registration']) + 1;
    }
} else {
    // This else block handles query failures; $next_registration_no remains 1 if no valid result
    error_log(sprintf("Error fetching max registration number for staff or table is empty: %s", (string)$conn->error));
}
...
.form-buttons button[type="button"]:hover {
    ...
    <script>
        function selectUser(registrationNo, staffId, username, lastName, firstName, middleName, email, department, accountType) {
