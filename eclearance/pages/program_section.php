...
$section_query = "SELECT s.*, l.LevelName 
                 FROM sections s 
...
if (isset($_POST['add_section'])) {
    try {
        if (empty($_POST['section_code']) || empty($_POST['section_title']) || empty($_POST['year_level'])) {
            throw new Exception("Please fill in all required fields.");
        }
        ...
        $stmt = $conn->prepare($insert_query);
        $stmt->bind_param("ssi", $section_code, $section_title, $year_level);
...
if (isset($_POST['edit_section'])) {
    try {
        ...
        // Update section
        $update_query = "UPDATE sections SET SectionCode = ?, SectionTitle = ?, YearLevel = ? WHERE SectionCode = ?";
        $stmt = $conn->prepare($update_query);
        $stmt->bind_param("ssis", $section_code, $section_title, $year_level, $original_code);
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        while ($section = $section_result->fetch_assoc()) { ?>
                            <tr onclick="selectSection('<?php echo htmlspecialchars($section['SectionCode']); ?>', '<?php echo htmlspecialchars($section['SectionTitle']); ?>', <?php echo $section['YearLevel']; ?>)">
